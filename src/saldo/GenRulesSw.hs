...
-------------------------------------------------------------------------
module GenRulesSw (
		   e, apply_suffixes, Suffixes,ins,
                   dv, dvu,keep,dsuff,
                   drop_last_vowel,
                   ds,dses, ds_drop,fl,ffst,
                     compound_s,
                   -- drop_last_vowel,
                    lift,
                    is_vowel,
...
dses :: String -> String
dses s = case reverse s of
         (x:y:_) | ((is_consonant x) && y == 's') || ((is_consonant x) && y == 'x') || [y,x] == "ch" || x=='x' || x =='z' || x =='s' -> s ++ "es"
         [] -> []
         _   -> s +? "s"
