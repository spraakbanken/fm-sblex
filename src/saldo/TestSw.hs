...
neg_gen_rules = [three_of_same, symbols, mmn,mmr,nnt,sms_missing_hyphen,end_with_h, multi_word_no_cmp]
  -- , multi_word_gen]
...
multi_word_no_cmp :: TestInput -> Result
multi_word_no_cmp t
 | not (is_multi_word t) = pass
 | (tpara t) == "nnm_iuc_pest_eller_kolera" = pass
 | not (is_a_word t && (param t ["ci"] || param t ["c"] || param t ["cm"])) = pass
 | otherwise = message t "multiword with compound forms"



-- multi_word_gen t
-- | is_nnm_first t && 
--   is_a_word t && param t ["gen"] = message t  "genitive in first nnm argument"
-- | otherwise = pass
